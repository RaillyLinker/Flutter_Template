...
class MainWidgetState extends State<MainWidget> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    mainBusiness.mainContext = context;
    mainBusiness.refreshUi = refreshUi;
    final InputVo? inputVo = mainBusiness.onCheckPageInputVo(
  ...
  Widget getScreenWidget() {
    ...
    return gw_slw_page_outer_frame.SlwPageOuterFrame(
      ...
      pageTitle: "YOLO 샘플",
      child: Center(
        // Fit both image and boxes to available space
        child: mainBusiness.originalImage == null
            ? CircularProgressIndicator()
            : FittedBox(
                fit: BoxFit.contain,
                child: SizedBox(
                  width: mainBusiness.originalImage!.width.toDouble(),
                  height: mainBusiness.originalImage!.height.toDouble(),
                  child: CustomPaint(
                    painter: DetectionPainter(
                      mainBusiness.uiImage!,
                      mainBusiness.detections,
                    ),
                  ),
                ),
              ),
...
class DetectionPainter extends CustomPainter {
  ...
  final List<main_business.Detection> detections;

  DetectionPainter(this.image, this.detections);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var det in detections) {
      canvas.drawRect(det.rect, paintBox);
      final textPainter = TextPainter(
        text: TextSpan(
          text: '${det.classId} ' + '${(det.score * 100).toStringAsFixed(1)}%',
          style: const TextStyle(color: Colors.red, fontSize: 12),
        ),
        textDirection: TextDirection.ltr,
      );
      textPainter.layout();
      textPainter.paint(canvas, det.rect.topLeft);
    }
  }
